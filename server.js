...
// ✅ 1. บอก Server ว่าไฟล์หน้าบ้าน (HTML/CSS/JS) อยู่ที่ไหน (สำคัญมาก)
...
// ✅ 2. Catch-All Route (วางไว้ล่างสุด)
// ใช้ app.use เพื่อดักทุก Route ที่ไม่ตรงกับข้างบน
app.use((req, res) => {
    const indexPath = path.join(__dirname, 'public', 'index.html');
    if (fs.existsSync(indexPath)) {
        res.sendFile(indexPath);
    } else {
        // ถ้าหาไฟล์ไม่เจอจริงๆ (เช่น public ว่างเปล่า)
        console.error("❌ Index file not found at:", indexPath);
        res.status(404).send('Error: public/index.html not found. Please check deployment.');
    }
});
