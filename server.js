...
const checkPlan = (requiredLevel) => {
    return async (req, res, next) => {
        ...
        if (userLvl >= reqLvl) { next(); } else { res.status(403).json({ message: `🔒 Upgrade to ${requiredLevel.toUpperCase()} to unlock this feature.` }); }
    };
};
...
// ✅ FIX: ปรับปรุง API extend-plan ให้รับ access array ด้วย
app.post('/api/admin/users/extend-plan', adminMiddleware, async (req, res) => { 
    try { 
        const { userId, plan, days, access } = req.body; 
        const user = await User.findById(userId); 
        
        user.plan = plan; 
        user.planExpire = new Date(new Date().getTime() + (days * 24 * 60 * 60 * 1000)); 
        
        // อัปเดตสิทธิ์ถ้ามีการส่งมา
        if (access && Array.isArray(access)) {
            user.access = access;
        }

        await user.save(); 
        res.json({ status: 'Success', message: `Updated ${user.name}: Plan, Days & Access Rights.` }); 
    } catch (e) { 
...
app.post('/api/me/test-email', authMiddleware, async (req, res) => { try { const userEmail = req.user.email; await sendEmailNotify(userEmail, "🔔 Quinn AI Test", `<h2>ทดสอบระบบแจ้งเตือนสำเร็จ!</h2><p>สวัสดีคุณ ${req.user.name}, นี่คืออีเมลทดสอบจากระบบ Quinn AI</p>`); res.json({ status: 'Success', message: `ส่งอีเมลทดสอบไปที่ ${userEmail} แล้ว` }); } catch (e) { res.status(500).json({ message: 'Email Error: ' + e.message }); } });
app.post('/api/me/update-profile', authMiddleware, async (req, res) => { try { const { password } = req.body; const user = await User.findById(req.user.id); let requireLogin = false; if (password) { user.password = await bcrypt.hash(password, 10); requireLogin = true; } await user.save(); res.json({ status: 'Success', message: 'อัปเดตข้อมูลสำเร็จ', requireLogin }); } catch (e) { res.status(500).json({ message: e.message }); } });
app.post('/api/auth/send-verification', authMiddleware, async (req, res) => { try { const user = await User.findById(req.user.id); if (user.isVerified) return res.status(400).json({ message: 'อีเมลนี้ยืนยันไปแล้ว' }); const token = crypto.randomBytes(32).toString('hex'); user.verificationToken = token; await user.save(); const link = `${process.env.FB_CALLBACK_URL.replace('/api/facebook/callback', '')}/verify-email.html?token=${token}`; await sendEmailNotify(user.email, "ยืนยันอีเมล Quinn AI", `<p>กรุณากดลิงก์ด้านล่างเพื่อยืนยันอีเมล:</p><a href="${link}">ยืนยันอีเมล</a>`); res.json({ status: 'Success', message: 'ส่งอีเมลยืนยันแล้ว กรุณาเช็ค Inbox' }); } catch (e) { res.status(500).json({ message: e.message }); } });
app.post('/api/auth/verify', async (req, res) => { try { const { token } = req.body; const user = await User.findOne({ verificationToken: token }); if (!user) return res.status(400).json({ message: 'Token ไม่ถูกต้อง' }); user.isVerified = true; user.verificationToken = undefined; await user.save(); res.json({ status: 'Success', message: 'ยืนยันอีเมลสำเร็จ!' }); } catch (e) { res.status(500).json({ message: e.message }); } });
...
app.use((req, res) => { const indexPath = path.join(__dirname, 'public', 'index.html'); if (fs.existsSync(indexPath)) { res.sendFile(indexPath); } else { res.status(404).send('Error: public/index.html not found'); } });
