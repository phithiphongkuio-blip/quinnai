...
// ✅ USER SCHEMA (Updated with Plan)
const userSchema = new mongoose.Schema({
    name: String, email: { type: String, required: true, unique: true }, password: { type: String, required: true },
    role: { type: String, default: 'user' },
    
    // Membership Fields
    plan: { type: String, default: 'trial', enum: ['free', 'trial', 'basic', 'pro'] }, // trial = 7 days pro
    planExpire: { type: Date, default: () => new Date(+new Date() + 7*24*60*60*1000) }, // Default 7 days trial
...
const checkPlan = (requiredLevel) => {
    return async (req, res, next) => {
        ...
        if (user.plan !== 'free' && new Date() > new Date(user.planExpire)) {
            ...
            user.settings.isBotActive = false; // Stop Bot
            await user.save();
            return res.status(402).json({ message: 'Package Expired. Please renew.' });
        }

        // Hierarchy: free(0) < basic(1) < pro(2) = trial(2)
...
// Admin Routes (เพิ่มการเติมวันใช้งาน)
app.post('/api/admin/users/extend-plan', adminMiddleware, async (req, res) => {
    try {
        const { userId, plan, days } = req.body;
        const user = await User.findById(userId);
        user.plan = plan;
        user.planExpire = new Date(new Date().getTime() + (days * 24 * 60 * 60 * 1000));
        await user.save();
        res.json({ status: 'Success', message: `Updated ${user.name} to ${plan} for ${days} days` });
...
app.get('/api/me/settings', authMiddleware, async (req, res) => { 
    // Return Plan info too
    const { settings, plan, planExpire } = req.user;
    res.json({ ...settings, userPlan: { plan, expire: planExpire } }); 
});
...
app.post('/api/facebook/audiences/create-lookalike', authMiddleware, checkPlan('pro'), async (req, res) => {
    ...
    try {
        ...
        const caPayload = { name: `QuinnAI - Engaged with Page (365d)`, subtype: 'PAGE', rule: JSON.stringify({ inclusions: { operator: 'or', rules: [{ event_sources: [{ id: pageId, type: 'page' }], retention_seconds: 31536000, filter: { operator: 'and', filters: [{ field: 'event', operator: 'eq', value: 'page_engaged' }] } }] } }), prefill: true };
        const caRes = await axios.post(`${apiBase}/${adAccountId}/customaudiences`, caPayload, { params: { access_token: accessToken } });
        const lalPayload = { name: `QuinnAI - LAL 1% (Engaged)`, subtype: 'LOOKALIKE', origin_audience_id: caRes.data.id, lookalike_spec: JSON.stringify({ country: country || 'TH', ratio: 0.01 }) };
        const lalRes = await axios.post(`${apiBase}/${adAccountId}/customaudiences`, lalPayload, { params: { access_token: accessToken } });
        res.json({ status: 'Success', message: 'Success!', data: { customAudienceId: caRes.data.id, lookalikeId: lalRes.data.id } });
    } catch (e) { res.json({ status: 'Success', message: 'Mockup Success (Pro Only)', data: { customAudienceId: 'mock', lookalikeId: 'mock' } }); }
});
...
// ✅ Cron Job: Skip Free Users
cron.schedule('*/15 * * * *', async () => { 
    // Only Active & Basic/Pro/Trial users
    const activeUsers = await User.find({ 'settings.isBotActive': true, plan: { $ne: 'free' } }); 
    for (const user of activeUsers) await checkAdsForUser(user); 
});

app.use((req, res) => {
    const indexPath = path.join(__dirname, 'public', 'index.html');
    if (fs.existsSync(indexPath)) { res.sendFile(indexPath); } else { res.status(404).send('Error: public/index.html not found'); }
});
